...
const StepFive = ({ nextStep, prevStep, type, subType }) => {
  ...
  const getPossStatus = async () => {
    ...
    try {
      ...
      console.log('possesion status error ===>', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    if (validate()) {
      ...
      const apiData1 = {
        ...
        purchase_id: formState?.purchase_id,
      };
      ...
      const imagesApiCall = async (projectListingid) => {
        ...
        try {
          ...
          if (res?.data?.status) {
            if (formState['specifications'].length > 0) {
              ckEditorApiCall(projectListingid);
            } else if (formState['furnishedStatus'] === 'Furnished' && formState['furnishedName'].length > 0) {
              await furnishedApiCall(projectListingid)
            } else if (formState['video1'] && formState['video2']) {
              videosApiCall(projectListingid)
            } else {
        ...
        if (resApi1?.data?.status) {
          await apiData2.map(async (payload, i) => {
            ...
            try {
              ...
              console.log('listing data error ===>', error);
              toastError('Error in saving Project Data');
            }
          });
          if (formState['amenities_id'].length > 0) {
            await amenitiesApiCall(resApi1?.data?.data?.id);
          } else if (formState['special_feature_id'].length > 0) {
            await specialFeaturesApiCall(resApi1?.data?.data?.id);
          } else if (formState['bank_id'].length > 0) {
            banksApiCall(resApi1?.data?.data?.id)
          } else {
            imagesApiCall(resApi1?.data?.data?.id)
          }
        } else {
          toastError('Error in saving Project Data');
        }
  ...
  return (
    <div>
      ...
      {/* <div className="card">
        <div className="card-header">
          <h4 className="card-title">Contact Timings</h4>
        </div>
        <div className="card-body">
          <div className="row mb-3">
            <h1 class="unitclass mb-3">
              When would be a convenient time for you to connect with the buyer?
            </h1>
